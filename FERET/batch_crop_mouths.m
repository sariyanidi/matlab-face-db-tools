function batch_crop_mouths( dirName, positivesDir, negativesDir )
%\brief
...
%
    ...
    for i=1:length(allFileNames)
        ...
        positive = crop_mouth([dirName '/' fileName]);
...
function mouth = crop_mouth( imName )
%DRAW_FACE_IN_IMAGE Summary of this function goes here
    ...
    xDist = leftEye(1)-rightEye(1);
    yDist = leftEye(2)-rightEye(2);
    
    aci = atan2(yDist,xDist);
    aci_deg = (180 * aci) / pi;

    nokta = [leftEye; rightEye; noseTip; mouth];
    nokta = nokta_dondur(nokta', -aci, size(im,2), size(im,1))';
    im = imrotate(im, aci_deg, 'bicubic', 'crop');  
    mouth = mouth_from_coords(im, nokta(1,:), nokta(2,:), nokta(3,:), nokta(4,:));
   
end

function mouth = mouth_from_coords(im, leftEye, rightEye, noseTip, mouth)
% Crop mouth using eye coords and mouth coords
% @param array im = face image
% @param vector leftEye: center of left eye (not camera left, the actual 
%                        left eye ofthe person!)
% @param vector rigthEye: center of right eye (not camera right)
% @param vector noseTip: not used
% @param vector mouth: center of the mouth 

    %decide the width of the mouth using eye distance
    boxWidth = leftEye(1) - rightEye(1);
    
    mouthX1 = mouth(1)-boxWidth/2;
    mouthY1 = mouth(2)-boxWidth/2;
    
    mouthY1 = mouthY1+boxWidth/4;
    
    mouth = imcrop(im, [mouthX1 mouthY1 boxWidth boxWidth-2*boxWidth/4]);
 end
...
%     draw_rect(im, faceTopLeft, faceBottomRight);
    ...
    figure(1);
    imshow(positive);
    positive = imrotate(positive, theta);
    negatives = 0;
