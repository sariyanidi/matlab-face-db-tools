function batch_crop_rotated(inputDir, outputDir)
% Crop FERET faces, rotate them 360 degrees and save them in
% separate folders.
    ...
    for i=1:length(Rotations)
        % Save each rotated image set to a separate folder
        dirName = [outputDir filesep num2str(Rotations(i)) '_Deg_IPR'];
        if ( ~isdir(dirName) )
            mkdir(dirName);
            mkdir([dirName '/Original']);
            mkdir([dirName '/Resized']);
        end
        
        batch_crop( inputDir, Rotations(i), dirName );
    end
end

function batch_crop( dirName, degrees, positivesDir )
%\brief
...
%
    ...
    for i=1:length(allFileNames)
        ...
        [positive negatives] = draw_face_in_image([dirName '/' fileName], degrees);
        
        if max(positive(:)) > 0
            resized = imresize(positive, [24 24]);
            imwrite( positive, [positivesDir '/Original/' fileName '_cropped.png'] );
            imwrite( resized, [positivesDir '/Resized/' fileName '_cropped.png'] );
...
function [positive negatives] = draw_face_in_image( imName, degrees )
%DRAW_FACE_�N_�MAGE Summary of this function goes here
    ...
    [positive negatives] = rect_from_coords(im, leftEye, rightEye, noseTip, mouth, degrees);
end

function [ positive negatives faceTopLeft faceBottomRight ] = rect_from_coords( im, leftEye, rightEye, noseTip, mouth, degrees )
...
%     draw_rect(im, faceTopLeft, faceBottomRight);
    ...
    try
        positive = imrotate(positive, -degrees, 'bilinear', 'crop');
        
        rn = round(15*normrnd(0,0.30)); % Add an artificial random rotation
        ...
        positive = imrotate(positive, rn, 'bilinear', 'crop');  
    catch exception
...
%   Detailed explanation goes here
    ...
    if ( isempty(leftEye) || isempty(rightEye) || isempty(noseTip) || isempty(mouth) )
        disp(sprintf('Insufficient DATA for image %s.tif! Image is passed by...', imName));
        leftEye = -1;
        return;
    end
